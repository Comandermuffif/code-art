    ...
    def draw(self, context:cairo.Context, color_mode:ColorMode, width:int, height:int) -> None:
        ...
        for count_x in range(self.count):
            for count_y in range(self.count):
                ...
                if count_y % 2 == 0:
                    color = color_mode.get_color((count_x + center_a[0])/self.count, (count_y + center_a[1])/self.count)
                    ...
                    color = color_mode.get_color((count_x + center_b[0])/self.count, (count_y + center_b[1])/self.count)
                    context.set_source_rgb(color.r, color.g, color.b)
